    ...
    useEffect(() => {
        if (reportType) {
          ...
          setFormData({
            ...
            runDate: reportType.runDate ? new Date(reportType.runDate).toISOString().split('T')[0] : '',
      ...
      const handleSave = async () => {
        const { id, name, outputType, frequency, runDate, runTime, emailsToNotify } = formData;
...
const renderTimePicker = (fieldId: string, value: string) => (
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            setReportTypes((prev) =>
                prev.map((conn) => (conn.id === reportTypeId ? { ...conn, ...response.data } : conn))
            );            

        } catch (error: any) {
            toast({
                title: "Error",
                description: `Failed to update connection: ${error.response?.data || error.message}`,
            });
        }
    };
    ...
    return (
        <div className="mt-2 p-2 sm:p-4">
          ...
          <SheetContent className="w-full sm:w-[90%] md:w-[70%] lg:w-[40%] max-w-lg">
            <SheetHeader>
              <SheetTitle className="text-lg sm:text-xl">{reportTypeId ? 'Edit Report Type' : 'Add New Report Type'}</SheetTitle>
  ...
  <div className="col-span-1 sm:col-span-2 flex flex-col gap-2">
    ...
    <input
      ...
      onChange={(e) => setFormData({ ...formData, emailsToNotify: e.target.value })}
    />
  </div>
</div>
...
{/* <div className="mt-4">
      ...
      {/* {formData.date && (
        ...
        </Sheet><div className="flex justify-end mb-2 space-x-4">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full border border-gray-200 min-w-[600px]">
                            ...
                            <tr className="bg-gray-100">
                                ...
                                <th className="px-2 sm:px-4 py-2 text-left text-xs sm:text-sm">Output Type</th>
                                <th className="px-2 sm:px-4 py-2 text-left text-xs sm:text-sm">Frequency</th>
                                <th className="px-2 sm:px-4 py-2 text-right text-xs sm:text-sm">Actions</th>
                            </tr>
                            </thead>
                            <tbody>
                            {reportTypes.length > 0 ? (
                                reportTypes.map((reportType) => (   
                                    <tr key={reportType.id} className="border-t">
                                        <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm">{reportType.name}</td>
                                        <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm">{mapOutputType(reportType.outputType)}</td>
                                        <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm">{mapFrequency(reportType.frequency)}</td>
                                        <td className="px-2 sm:px-4 py-2 text-right">
                                            <div className="flex flex-col sm:flex-row justify-end space-y-1 sm:space-y-0 sm:space-x-2">
                                                <button
                                                    onClick={() =>{ handleEditReportType(reportType)
                                                        setIsOpen(true);
                                                    }}
                                ...
                                <tr>
                                    <td colSpan={4} className="text-center py-4 text-sm">No report types available</td>
                                </tr>
                            )}
                            </tbody>
                        </table>
