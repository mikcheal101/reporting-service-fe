...
describe('useScheduleReport', () => {
  ...
  it('should show success toast when scheduling succeeds', async () => {
    (api.post as ReturnType<typeof vi.fn>).mockResolvedValue({ data: true });

    const { result } = renderHook(() => useScheduleReport(), { wrapper: createWrapper() });
  ...
  it('should show destructive toast when scheduling returns false', async () => {
  ...
  it('should show error toast on exception', async () => {
    (api.post as ReturnType<typeof vi.fn>).mockRejectedValue({
      response: { data: { message: 'Schedule failed' } },
    });
